...
export default function SalesDocumentPage() {
            ...
            mutateDocument(
              (document) => {
                if (document) {
                  ...
                  return { ...document, items: newItems };
                }
              },
              {
                revalidate: false,
          ...
          onChange: (row: number, value: string | number) => {
            mutateDocument(
              (document) => {
                if (document) {
                  const newItems = [...(document?.items || [])];
                  newItems[row]["quantity"] = Number(value);
                  console.log(newItems);
                  return { ...document, items: newItems };
                }
              },
  ...
  return (
    <main className="p-2 h-full flex flex-col">
      ...
      <h1 className="flex text-2xl pb-3">Продаж товарів</h1>
      <div className="flex flex-col gap-2 bg-gray-50">
        ...
        <div className="flex gap-2">
          <FormControl variant="outlined" size="small">
            <InputLabel id="supplier-select-lable">Клієнт</InputLabel>
            <Select
              ...
              onChange={(e) =>
                mutateDocument(
                  (p) => (p ? { ...p, client: e.target.value } : undefined),
                  { revalidate: false },
                )
              }
          ...
          <TextField
            ...
            onChange={(e) =>
              mutateDocument(
                (p) => (p ? { ...p, comment: e.target.value } : undefined),
                { revalidate: false },
              )
            }
          />
