...
// 		const stock_items = $app.findRecordsByFilter("product_stock", `document = '${record.id}' && document_type = 'income'`);
// 		for (let i = 0; i < stock_items.length; i++) {
// 			const item = stock_items[i];
// 			$app.delete(item);
// 	    }
// 	    if(record.posted){
// 			const document_items = $app.findRecordsByFilter("income_document_product", `document = '${record.id}'`);
...
// 				stockRecord.set("document",record.id);
